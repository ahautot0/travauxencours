...
int main()
{long long int i,quo, nb, nx, resu[321]={0}; 
int S=4,halt=0,j=0,k,ipas,jj=0,j1,nbfig[2*4+1],state,pos,u,posmax,tripl,tape[170],return1;
bool return2;
...
for (nb = 0; nb < 4294967296; nb+=2) {sym4(nb,return1,return2);
	...
	memset(tape, 0, sizeof(tape[0]) * 170);/*for (k=0;k<170;k++) tape[k]=0;*/  pos=1;state=tripl=0;ipas=1;posmax=1;    //initialisation de la machine

while (pos>0  && ipas<170) { state=tripl/4; tripl=nbfig[2*S-2*state-tape[pos]]; tape[pos]=(tripl/2) % 2; pos=pos+1-2*(tripl % 2); if (pos>posmax) posmax=pos; ipas++;}
